...
(function(b) {
    ...
    b.fn.uploadFile = function(m) {
        var l = b.extend({url: "", method: "POST", enctype: "multipart/form-data", formData: null, returnType: null, allowedTypes: "*", fileName: "file", multiple: true, dragDrop: true, autoSubmit: true, showCancel: true, showAbort: true, showDone: true, showStatusAfterSuccess: true, showStatusAfterError: true, buttonCss: false, buttonClass: false, onSubmit: function(n) {
            }, onSuccess: function(o, n, p) {
            }, onError: function(p, n, o) {
            }, uploadButtonClass: "ajax-file-upload", dragDropStr: "<span><b>Drag &amp; Drop Files</b></span>"}, m);
        this.fileCounter = 1;
        if (!a.formdata) {
            l.dragDrop = false
        }
        ...
        b(e).addClass(l.uploadButtonClass);
        if (l.dragDrop) {
            var g = b('<div class="ajax-upload-dragdrop"></div>');
            b(this).before(g);
            b(g).append(e);
            b(g).append(b(l.dragDropStr));
            f(this, l, g)
        } else {
            b(this).before(e)
        }
        ...
        this.startUpload = function() {
            b("." + this.formGroup).each(function(o, n) {
                b(this).submit()
            })
        };
        b(this).hide();
        function f(p, n, o) {
            o.on("dragenter", function(q) {
                q.stopPropagation();
                q.preventDefault();
                b(this).css("border", "2px solid #A5A5C7")
            });
            o.on("dragover", function(q) {
                q.stopPropagation();
                q.preventDefault()
            });
            o.on("dragexit", function(q) {
                q.stopPropagation();
                q.preventDefault()
            });
            o.on("drop", function(r) {
                b(this).css("border", "2px dotted #A5A5C7");
                r.preventDefault();
                var q = r.originalEvent.dataTransfer.files;
                h(n, p, q)
            });
            b(document).on("dragenter", function(q) {
                q.stopPropagation();
                q.preventDefault()
            });
            b(document).on("dragover", function(q) {
                q.stopPropagation();
                q.preventDefault()
            });
            b(document).on("dragexit", function(q) {
                q.stopPropagation();
                q.preventDefault()
            });
            b(document).on("drop", function(q) {
                q.preventDefault()
            })
        }
        function i(r) {
            ...
            for (q = 0; q < o; q++) {
                s[q] = s[q].replace(/\+/g, " ");
                p = s[q].split("=");
                n.push([decodeURIComponent(p[0]), decodeURIComponent(p[1])])
            }
            return n
        }
        function h(y, t, n) {
            for (var u = 0; u < n.length; u++) {
                if (c(t, y, n[u].name)) {
                    var v = y;
                    var p = new FormData();
                    p.append(y.fileName, n[u]);
                    if (y.formData) {
                        var x = i(y.formData);
                        for (var r = 0; r < x.length; r++) {
                            if (x[u]) {
                                p.append(x[r][0], x[r][1])
                            }
                        }
                    }
                    v.fileData = p;
                    var w = new j(t);
                    w.filename.html(t.fileCounter + "). " + n[u].name);
                    var o = b("<form style='display:block; position:absolute;left: 150px;' class='" + t.formGroup + "' method='" + y.method + "' action='" + y.url + "' enctype='" + y.enctype + "'></form>");
                    o.appendTo("body");
                    var q = [];
                    q.push(n[u].name);
                    b.fn.uploadFile.ajaxFormSubmit(o, v, w, q);
                    t.fileCounter++
                }
            }
        }
        function c(p, o, r) {
            var q = o.allowedTypes.toLowerCase().split(",");
            var n = r.split(".").pop().toLowerCase();
            if (o.allowedTypes != "*" && jQuery.inArray(n, q) < 0) {
                return false
            }
            return true
        }
        function k(u, t, p, n) {
            var v = "ajax-upload-id-" + (new Date().getTime());
            var r = b("<form method='" + p.method + "' action='" + p.url + "' enctype='" + p.enctype + "'></form>");
            var o = "<input type='file' id='" + v + "' name='" + p.fileName + "'/>";
            if (p.multiple) {
                if (p.fileName.indexOf("[]") != p.fileName.length - 2) {
                    p.fileName += "[]"
                }
                o = "<input type='file' id='" + v + "' name='" + p.fileName + "' multiple/>"
            }
            var q = b(o).appendTo(r);
            q.change(function() {
                var B = p.allowedTypes.toLowerCase().split(",");
                var x = [];
                if (this.files) {
                    for (y = 0; y < this.files.length; y++) {
                        if (!c(u, p, this.files[y].name)) {
                            alert("File type is not allowed. Allowed only: " + p.allowedTypes);
                            return
                        }
                        x.push(this.files[y].name)
                    }
                } else {
                    var z = b(this).val();
                    x.push(z);
                ...
                if (p.multiple && a.formdata) {
                    ...
                    for (var y = 0; y < x.length; y++) {
                        w += u.fileCounter + "). " + x[y] + "<br>";
                        u.fileCounter++
                    }
                    var s = new j(u);
                    s.filename.html(w);
                    b.fn.uploadFile.ajaxFormSubmit(r, p, s, x)
                }
            });
            r.css({margin: 0, padding: 0, display: "block", position: "absolute", left: "-550px"});
            r.appendTo("body");
